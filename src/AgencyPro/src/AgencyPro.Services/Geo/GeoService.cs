...
namespace AgencyPro.Services.Geo
{
    public class GeoService : Service<Country>, IGeoService
    {
        public GeoService(IServiceProvider serviceProvider)
        ...
        ///     1. If there is already a default language setup and the coming is default, set the old as NOT default and the new
        ///     as default
        /// </summary>
        ...
        public async Task SetCountryLanguage(string iso2, string languageCode, bool main)
        {
            ...
            if (!entries.Any())
            {
                await repository.InsertAsync(new LanguageCountry
                    {Iso2 = iso2, LanguageCode = languageCode, Default = main});
            }
            else
            {
                ...
                if (defaultOne != null && main)
                {
                    ...
                    if (defaultOne.LanguageCode.Equals(languageCode)) return;
